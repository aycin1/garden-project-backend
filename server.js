...
app.get("/shopping-list/:user", handleGetShopping);
...
async function handleGetShopping(req, res) {
  ...
  const response = (
    await client.query(
      `SELECT * FROM shopping_list JOIN gardens ON gardens.id = shopping_list.garden_id WHERE user_id = ? ORDER BY plant_info_id`,
      [userID]
    )
  ).rows;
